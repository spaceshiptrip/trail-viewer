...
async function makeTempRepo() {
  const root = await fs.mkdtemp(
    path.join(os.tmpdir(), "trail-viewer-add-track-"),
  );
  await fs.mkdir(path.join(root, "scripts"), { recursive: true });
  await fs.mkdir(path.join(root, "public", "tracks", "gpx"), {
    recursive: true,
  });
  return root;
}
...
<gpx version="1.1" creator="test" xmlns="http://www.topografix.com/GPX/1/1">
  <trk>
    <name>Sample</name>
    <trkseg>
      <trkpt lat="34.1000" lon="-117.1000"><ele>100.0</ele></trkpt>
      <trkpt lat="34.2000" lon="-117.2000"><ele>150.0</ele></trkpt>
    </trkseg>
  </trk>
</gpx>`,
    "utf8",
  ...
  assert.equal(path.basename(result.gpxPath), "sample-input.gpx");
  assert.equal(path.basename(result.geojsonPath), "sample-input.geojson");
