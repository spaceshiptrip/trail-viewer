...
export default function DrawTrailMode({ onSave, onClose }) {
  ...
  const snapToRoads = async () => {
    ...
    try {
      ...
      // Use OSRM match service for map matching
      const response = await fetch(
        `${OSRM_SERVER}/match/v1/foot/${coordinates}?overview=full&geometries=geojson&steps=true&annotations=true`
      );
      ...
      if (data.code !== 'Ok' || !data.matchings || data.matchings.length === 0) {
        throw new Error('Could not match route to roads. Try drawing points closer together.');
      }
      
      // Get the matched route geometry
      const matching = data.matchings[0];
      const routeCoords = matching.geometry.coordinates;
      
      // Convert to [lat, lon] format and add elevation (if available)
      const snappedCoords = routeCoords.map(coord => {
        // OSRM returns [lon, lat], we need [lon, lat, elevation]
        // We'll fetch elevation separately or leave it as 0
      ...
      <div className="absolute top-4 right-4 z-[1000] w-80 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg shadow-xl p-4">
        ...
        <div className="mb-4 p-3 bg-[var(--bg-tertiary)] rounded-lg text-sm text-[var(--text-secondary)]">
          {!isDrawing && drawnPoints.length === 0 && (
            <p>Click "Start Drawing" then click on the map to create points along your trail.</p>
          )}
          {isDrawing && (
            <p>Click on the map to add points. Click "Snap to Road" when done.</p>
          )}
          {snappedRoute && (
            <p>✓ Route snapped! Enter details and save as GeoJSON or GPX.</p>
          )}
        </div>
            ...
            {!isDrawing ? (
              ...
              <>
                ...
                >
                  ...
                  {loading ? 'Snapping to Road...' : 'Snap to Road'}
                </button>
                <button
