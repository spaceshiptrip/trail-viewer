...
// Component to fit map bounds to all tracks
...
export default function Map({ tracks, selectedTrack, onTrackClick }) {
  ...
  const getTrackStyle = (feature) => {
    ...
    return {
      ...
      // color: isSelected ? '#5ab887' : '#B85A8B',
      // color: isSelected ? '#5ab887' : '#5A8BB8',
    ...
    layer.on({
      ...
      mouseover: (e) => {
        if (!selectedTrack || selectedTrack.properties.id !== feature.properties.id) {
          ...
          e.target.setStyle({ color: '#5A8BB8', weight: 8, opacity: 0.9 });
        }
      },
      mouseout: (e) => {
        if (!selectedTrack || selectedTrack.properties.id !== feature.properties.id) {
